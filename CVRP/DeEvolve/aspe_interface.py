...
class ASPE_Interface:
    ...
    def generate_new_individual(self, program_info):
        ...
        try:
            ...
            response1 = self.prompts.get_sub_prob("CVRP", program_info)
            program_info.update(self.prompts.extract_sub_problems(response1))
            # Step 2: Select sub-problem to improve
            response2 = self.prompts.get_select_prob("CVRP", program_info)
            program_info.update(self.prompts.extract_sub_problem(response2))

            #response3 = self.prompts.get_alg_ref("CVRP", program_info)
            #print(response3)
            ...
            response5 = self.prompts.get_code_imp("CVRP", program_info)
            ...
            while m < 5:
                try:
                    ...
                    response_debug = self.prompts.get_code_debug("CVRP", program_info)
                    ...
                    m += 1
            program_info['score'] = 60000
            program_info['time'] = 60000
            return program_info
