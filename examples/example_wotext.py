...
def solvetsp(C, distances_truck, truck_speed):
    if len(C) <= 10:
        ...
        for perm in permutations(C):
            current_route = [depot] + list(perm) + [depot]
            current_distance = 0

            for i in range(len(current_route) - 1):
                current_distance += distances_truck[current_route[i]][current_route[i + 1]]
        ...
        for i in range(1, len(best_route)):
            t[best_route[i]] = t[best_route[i - 1]] + distances_truck[best_route[i - 1]][best_route[i]] / truck_speed

        return best_route, t
    else:
        # Improved algorithm for larger instances using Christofides with 2-opt
        G = nx.Graph()
        depot = 0
        nodes = [depot] + C
        ...
        # Get initial solution using Christofides
        initial_route = approximation.christofides(G, weight='weight')
        ...
        optimized_route = two_opt(initial_route.copy())

        # Calculate arrival times
        t = {0: 0}
        for i in range(1, len(optimized_route)):
            t[optimized_route[i]] = t[optimized_route[i - 1]] + distances_truck[optimized_route[i - 1]][
                optimized_route[i]] / truck_speed

        return optimized_route, t
